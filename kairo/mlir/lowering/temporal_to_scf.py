...
class TemporalToSCFPass:
    ...
    def _process_operation(self, op: Any) -> None:
        ...
        if TemporalDialect.is_temporal_op(op):
            ...
            if op_name == "kairo.temporal.flow.create":
                self._lower_flow_create(op)
            elif op_name == "kairo.temporal.flow.run":
                self._lower_flow_run(op)
            elif op_name == "kairo.temporal.state.create":
                self._lower_state_create(op)
            elif op_name == "kairo.temporal.state.update":
                self._lower_state_update(op)
            elif op_name == "kairo.temporal.state.query":
...
def create_temporal_to_scf_pass(context: KairoMLIRContext) -> TemporalToSCFPass:
