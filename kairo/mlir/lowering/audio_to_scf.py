...
class AudioToSCFPass:
    ...
    def _process_operation(self, op: Any) -> None:
        ...
        if AudioDialect.is_audio_op(op):
            ...
            if op_name == "kairo.audio.buffer.create":
                self._lower_buffer_create(op)
            elif op_name == "kairo.audio.oscillator":
                self._lower_oscillator(op)
            elif op_name == "kairo.audio.envelope":
                self._lower_envelope(op)
            elif op_name == "kairo.audio.filter":
                self._lower_filter(op)
            elif op_name == "kairo.audio.mix":
                self._lower_mix(op)
...
def create_audio_to_scf_pass(context: KairoMLIRContext) -> AudioToSCFPass:
