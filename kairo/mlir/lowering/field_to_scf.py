...
class FieldToSCFPass:
    ...
    def _process_operation(self, op: Any) -> None:
        ...
        if FieldDialect.is_field_op(op):
            ...
            if op_name == "kairo.field.create":
                self._lower_field_create(op)
            elif op_name == "kairo.field.gradient":
                self._lower_field_gradient(op)
            elif op_name == "kairo.field.laplacian":
                self._lower_field_laplacian(op)
            elif op_name == "kairo.field.diffuse":
...
def create_field_to_scf_pass(context: KairoMLIRContext) -> FieldToSCFPass:
